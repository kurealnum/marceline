#!/usr/bin/env python3
"""Python STT worker (SPEC.md §2.2/§2.3/§2.4.1, EPIC 3.1).

A long-lived subprocess managed by the Rust core, hosting a Whisper-family
model and serving the streaming `marceline.stt.Stt` contract over a local
unix domain socket. Built on the worker template (EPIC 0.4): same CLI
convention, same `grpc.health.v1.Health` service the supervisor
(EPIC 0.6) pings.

The model lives here, out of process, so a CUDA OOM kills a worker the
supervisor can restart instead of taking down the daemon (§2.2). Swapping
models is a restart with a different `--model-id`, not a code change
(EPIC 3.4).
...
Whisper window fills or the request stream ends, at which point that
buffer is transcribed and a single `final` is streamed back. A `Cancel`
message interleaved on the request stream is read by a reader thread
while inference is running, and the generate loop notices it between
decode steps (§2.5.1).
"""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Marceline STT worker")
    parser.add_argument(
        "--socket-path",
        required=True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Imported here, not at module scope, so the heavy ML stack is loaded
    # only by the real worker process — the servicer's own tests inject a
    # stub backend and need neither torch nor transformers.
    from whisper_backend import WhisperBackend

    backend = WhisperBackend(args.model_id, args.device, args.lang)
